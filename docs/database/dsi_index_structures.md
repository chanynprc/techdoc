...
- 当每个块能容纳的键数n足够大时，分裂和合并块的情况会很少，所以B树重组的IO开销较小
- 因n足够大，B数索引的层数也不会太深，所以一次读取不会耗费太多的IO
- 可通过缓存B树某些节点的方式，进一步减少磁盘IO
- 在删除元素时，可以暂不对B树节点记录数小于最小值的情况进行修复，可能后续会插入数据，很快记录数又会再次达到键-指针对的最小数
